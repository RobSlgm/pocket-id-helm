...
> [!WARNING]
> Changes to the `blobBackend` (environment setting `FILE_BACKEND`) require a data migration.
...
```
helm upgrade my-release oci://ghcr.io/robslgm/charts/pocket-id --set image.tag=v2.14.0
```
...
Pocket ID 2.2+ has an **experimental** import and export function (works with distroless).
